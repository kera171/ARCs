...
fouling_test10_25_24.ARC1b <- fouling_test10_25_24.ARC1b %>% 
  ...
  filter(datetime>="2024-11-01 08:00")
...
fouling_test10_25_24.ARC3 <- fouling_test10_25_24.ARC3 %>% 
  ...
  filter(datetime<="2024-11-08 07:00")
...
fouling.all.10_25_24 %>% 
  ...
  theme_classic()+
  basic_theme+
  labs(x="",y= expression(paste("Respiration Rate (","mmol ","O"^"2"," m"^"-3"," hr"^"-1",")")))
...
ARC1b.closest.time.first.half <- ARC1b.closest.time %>% 
  filter(datetime<="2024-11-05 00:00") %>% 
  filter(datetime>="2024-11-01 08:00")
ARC1b.closest.time.second.half <- ARC1b.closest.time %>% 
  filter(datetime>="2024-11-05 00:00")

ARC1b.closest.time <- ARC1b.closest.time %>% 
  filter(datetime>="2024-11-01 08:00")
...
mean.diff.1b <- mean(ARC1b.closest.time$normalized.diff.1,na.rm = TRUE)

mean.diff.2b <- mean(ARC1b.closest.time$normalized.diff.2,na.rm = TRUE)

#finding the standard deviation of the differences for each paired deployments with 1b

sd.diff.1b <- sd(ARC1b.closest.time$normalized.diff.1,na.rm = TRUE)

sd.diff.2b <- sd(ARC1b.closest.time$normalized.diff.2,na.rm = TRUE)

#getting the number of values that are not NA for each difference column

n.diff.1b <- nrow(ARC1b.closest.time[!is.na(ARC1b.closest.time$normalized.diff.1), ])

n.diff.2b <- nrow(ARC1b.closest.time[!is.na(ARC1b.closest.time$normalized.diff.2), ])

#determining the critical value and the confidence intervals 


cl.diff.1b <- c(mean.diff.1b + qt(0.025,n.diff.1b-1)*sd.diff.1b/sqrt(n.diff.1b),
               mean.diff.1b + qt(0.975,n.diff.1b-1)*sd.diff.1b/sqrt(n.diff.1b))

cl.diff.2b <- c(mean.diff.2b + qt(0.025,n.diff.2b-1)*sd.diff.2b/sqrt(n.diff.2b),
               mean.diff.2b + qt(0.975,n.diff.2b-1)*sd.diff.2b/sqrt(n.diff.2b))

t.test(ARC1b.closest.time$normalized.diff.1,alternative = "less", conf.level = 0.95)

t.test(ARC1b.closest.time$normalized.diff.2,alternative = "less", conf.level = 0.95)


```
One Sample t-test

data:  ARC1b.closest.time$normalized.diff.1
t = -0.40818, df = 26, p-value = 0.3432
...
data:  ARC1b.closest.time$normalized.diff.2
t = -2.2649, df = 27, p-value = 0.01588
alternative hypothesis: true mean is less than 0
95 percent confidence interval:
        -Inf -0.01509648
sample estimates:
...
```{r  normalized differences for second swap first half}
#finding the mean of the differences for each paired deployments with 1b

mean.diff.1b.fh <- mean(ARC1b.closest.time.first.half$normalized.diff.1,na.rm = TRUE)

mean.diff.2b.fh <- mean(ARC1b.closest.time.first.half$normalized.diff.2,na.rm = TRUE)

#finding the standard deviation of the differences for each paired deployments with 1b

sd.diff.1b.fh <- sd(ARC1b.closest.time.first.half$normalized.diff.1,na.rm = TRUE)

sd.diff.2b.fh <- sd(ARC1b.closest.time.first.half$normalized.diff.2,na.rm = TRUE)

#getting the number of values that are not NA for each difference column

n.diff.1b.fh <- nrow(ARC1b.closest.time.first.half[!is.na(ARC1b.closest.time.first.half$normalized.diff.1), ])

n.diff.2b.fh <- nrow(ARC1b.closest.time.first.half[!is.na(ARC1b.closest.time.first.half$normalized.diff.2), ])

#determining the critical value and the confidence intervals 


cl.diff.1b.fh <- c(mean.diff.1b.fh + qt(0.025,n.diff.1b.fh-1)*sd.diff.1b.fh/sqrt(n.diff.1b.fh),
               mean.diff.1b.fh + qt(0.975,n.diff.1b.fh-1)*sd.diff.1b.fh/sqrt(n.diff.1b.fh))

cl.diff.2b.fh <- c(mean.diff.2b.fh + qt(0.025,n.diff.2b.fh-1)*sd.diff.2b.fh/sqrt(n.diff.2b.fh),
               mean.diff.2b.fh + qt(0.975,n.diff.2b.fh-1)*sd.diff.2b.fh/sqrt(n.diff.2b.fh))

t.test(ARC1b.closest.time.first.half$normalized.diff.1,alternative = "less", conf.level = 0.95)

t.test(ARC1b.closest.time.first.half$normalized.diff.2,alternative = "less", conf.level = 0.95)


```

```{r  normalized differences for second swap second half}
#finding the mean of the differences for each paired deployments with 1b

mean.diff.1b.sh <- mean(ARC1b.closest.time.second.half$normalized.diff.1,na.rm = TRUE)

mean.diff.2b.sh <- mean(ARC1b.closest.time.second.half$normalized.diff.2,na.rm = TRUE)

#finding the standard deviation of the differences for each paired deployments with 1b

sd.diff.1b.sh <- sd(ARC1b.closest.time.second.half$normalized.diff.1,na.rm = TRUE)

sd.diff.2b.sh <- sd(ARC1b.closest.time.second.half$normalized.diff.2,na.rm = TRUE)

#getting the number of values that are not NA for each difference column

n.diff.1b.sh <- nrow(ARC1b.closest.time.second.half[!is.na(ARC1b.closest.time.second.half$normalized.diff.1), ])

n.diff.2b.sh <- nrow(ARC1b.closest.time.second.half[!is.na(ARC1b.closest.time.second.half$normalized.diff.2), ])

#determining the critical value and the confidence intervals 


cl.diff.1b.sh <- c(mean.diff.1b.sh + qt(0.025,n.diff.1b.sh-1)*sd.diff.1b.sh/sqrt(n.diff.1b.sh),
               mean.diff.1b.sh + qt(0.975,n.diff.1b.sh-1)*sd.diff.1b.sh/sqrt(n.diff.1b.sh))

cl.diff.2b.sh <- c(mean.diff.2b.sh + qt(0.025,n.diff.2b.sh-1)*sd.diff.2b.sh/sqrt(n.diff.2b.sh),
               mean.diff.2b.sh + qt(0.975,n.diff.2b.sh-1)*sd.diff.2b.sh/sqrt(n.diff.2b.sh))

t.test(ARC1b.closest.time.second.half$normalized.diff.1,alternative = "less", conf.level = 0.95)

t.test(ARC1b.closest.time.second.half$normalized.diff.2,alternative = "less", conf.level = 0.95)
...
#2nd week differences
  plt.3 <- ggplot()+
  geom_ribbon(data = ARC1b.closest.time,
              aes(x=datetime,ymin = cl.diff.1b[1],
                            ymax=cl.diff.1b[2]),
              fill=pal[1],alpha=0.1)+
  ...
  plt.4 <- ggplot()+
  geom_ribbon(data = ARC1b.closest.time,
              aes(x=datetime,ymin = cl.diff.2b[1],
                            ymax=cl.diff.2b[2]),
              fill=pal[1],alpha=0.1)+
...
plt.10+plt.11+plot_layout(nrow = 1)+
  plot_annotation(tag_levels = "a")
...
mean.diff.1a <- mean(ARC1a_2nd.closest.time$normalized.diff,na.rm = TRUE)

#finding the standard deviation of the differences for each paired deployments with 1a_2nd

sd.diff.1a<- sd(ARC1a_2nd.closest.time$normalized.diff,na.rm = TRUE)

#getting the number of values that are not NA for each difference column

n.diff.1a <- nrow(ARC1a_2nd.closest.time[!is.na(ARC1a_2nd.closest.time$normalized.diff), ])

#determining the critical value and the confidence intervals 

cl.diff.1 <- c(mean.diff.1a + qt(0.025,n.diff.1a-1)*sd.diff.1a/sqrt(n.diff.1a),
               mean.diff.1a + qt(0.975,n.diff.1a-1)*sd.diff.1a/sqrt(n.diff.1a))
...
combined.plt <- plt.1+plt.2+plt.5+plt.3+plt.4+plt.6+plot_layout(nrow = 2)+
  plot_annotation(tag_levels = "a")

png(file = here("R data/Figures","Combined Normalized Difference Plot.png"), width=13,height=10,units="in",res=150, pointsize = 2)
...
t.test(ARC1a_2nd.closest.time$normalized.diff,alternative = "less", conf.level = 0.95)
...
data:  ARC1a_2nd.closest.time$normalized.diff
t = -3.6855, df = 26, p-value = 0.0005279
alternative hypothesis: true mean is less than 0
95 percent confidence interval:
       -Inf -0.1166889
sample estimates:
