...
full.swap <- read_excel("C:/Users/hunte/Desktop/R projects/ARCs/Raw data/ES_060524 full cleaned.xlsx",sheet = "Sheet1") %>% 
  mutate(datetime.2= as.POSIXct(paste(datetime),tz="America/New_York")) %>% 
  select(-datetime) %>% 
  rename(datetime=datetime.2)

second.swap <- read_excel("C:/Users/hunte/Desktop/R projects/ARCs/Raw data/ES_061324_cleaned.xlsx",sheet = "Sheet1") %>% 
  mutate(datetime.2= as.POSIXct(paste(datetime),tz="America/New_York")) %>% 
  select(-datetime) %>% 
  rename(datetime=datetime.2)
...
#source("ARC data Processing Functions.R")
```
