...
# gives voltage files as separate files
...
#rm(ARC01_MID4_VoltData,ARC02_MID4_VoltData,ARC03_MID4_VoltData,ARC04_MID4_VoltData,ARC05_MID4_VoltData,ARC06_MID4_VoltData,ARC07_MID4_VoltData,ARC01_MID4_Dark5ft,ARC02_MID4_Dark9ft,ARC03_MID4_Dark20ft,ARC04_MID4_Dark35ft,ARC05_MID4_Dark44ft,ARC06_MID4_Clear5ft,ARC07_MID4_Clear9ft)
```
...
voltage.filenames <- ls(pattern = "VoltData")
...
```{r}
# remove the data after recovery and before from ARC
...
test <- slice(df, 1:which(df$incubation.number==max(df$incubation.number,na.rm =TRUE) & abs(df$do.mgl[-1] - df$do.mgl) >= 0.2))

test <-  subset(df, incubation.number != 0) 
...
assign(paste0(title,".arcrates"),df.rates)
...
rate.filenames <- ls(pattern = "\\.arcrates")
...
for (i in seq_along(rate.files)){
  ...
  df <- rate.files[[i]]
  deployment <- format(df$datetime[i],format = "%Y-%m-%d")
  
save(df, file = here("R data/Rdata files",paste0(rate.filenames[i],deployment,".Rdata")))

write_xlsx(df, path = here("saved files",paste0(rate.filenames[i],deployment,".xlsx")), col_names = TRUE)
