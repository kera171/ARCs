...
# For Anna lisa:
setwd("C:/Users/amudahy/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Rdata")
#load("~/Rdata/arc2021excr.Rdata")
load("~/Rdata/arc2021wlis.Rdata")
load("~/Rdata/tide2021ARCS.Rdata")
names(arc2021WLIS) <- c ('Timestamp.Rates','Rates.WS','Rates.WD', 'surf.salinity',
...
arc2021WLIS$Timestamp.Rates <- as.Date.numeric(origin = "1900-01-01", arc2021WLIS$Timestamp.Rates, tz="America/Jamaica",usetz=TRUE) %>% as.POSIXct()

arc2021WLIS$Rates.WD <- as.numeric(arc2021WLIS$Rates.WD)
arc2021WLIS$mid.temp <- as.numeric(arc2021WLIS$mid.temp)
arc2021WLIS$surf.temp <- as.numeric(arc2021WLIS$surf.temp)

arc2021WLIS$tide.height.m <- approx(tide.2$TimeStamp.EST, tide.2$height.m,arc2021WLIS$Timestamp.Rates) %>%
  .[[2]]

# calculate predictor variables
arc2021WLIS<- arc2021WLIS%>% 
  mutate(
...
arc2021WLIS %>% ggplot(aes(tide.height.m,Rates.WD))+
...
arc2021WLIS %>% 
  filter(!is.na(month)) %>% 
  mutate(month=factor(month,labels=month.name[c(7,8,9,10)])) %>%  # set factor labls to month words
  ggplot(aes(x=hour,y=Rates.WD))+
  facet_wrap(~month)+
  geom_boxplot(fill=pal[2])+
  basic_theme+
  labs(x="Tide Height",y="Respioration Rate")
...
model.1 <- with(arc2021WLIS,gam(Rates.WS ~
                                    #s(hour,bs="cc",k=4) +
                                    s(surf.salinity)+
                                    s(mid.salinity)+
                                    s(surf.temp)+
...
# model 2
model.2 <- with(arc2021WLIS,gam(Rates.WD ~ 
                                    #S(hour,bs="cc") +
                                    s(surf.salinity)+
                                    s(mid.salinity)+
                                    s(deep.salinity)+
...
model.3<- with(arc2021WLIS,gam(Rates.WD ~ 
                                   s(surf.salinity)+
                                   s(mid.salinity,k=3)+
                                   s(mid.o2,k=3)+
                                   s(surf.o2)+
