#ARC Processing Functions with Voltage 05/22/2025
...
calculate_slope <- function(mydata) {
  ...
  slopes.o2.calc$n <- NA
  slopes.o2.calc$datetime <- as.POSIXct.default("1899-01-01 04:58:59", tz="America/New_York")
...
# graphing figure for rates must copy and paste this in to code to change the name pdf file------------
graph.figures.rates <-  function(mydata){
  
  incubations <- unique(mydata$incubation.number[!is.na(mydata$incubation.number)])
  
  pdf(file = paste0("R data/Figures/Figures ",mydata$deployment[1] ,format(mydata$datetime[1],format = "%Y-%m-%d" ),".pdf"), 
      width = 6, 
      height = 6,
      paper = "letter")
  
  for (j in 1:length(incubations)){
    df.segment <- mydata %>% 
      filter(incubation.number==incubations[j])
    plt <- ggplot( df.segment, aes(x=datetime,y=do.mgl))+
      ...
      xlab("")+
      ggtitle(paste("Incubation Number",incubations[j],"--",
