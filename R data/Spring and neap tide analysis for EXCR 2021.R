#----- Using Lunar to get spring/neap data
...
dates <- seq(as.Date("2021-06-01"),as.Date("2021-10-31"), by="day")
...
arc2021EXCR$dates <- as.Date.POSIXct(arc2021EXCR$Time_R,format("%Y-%m-%d"))


arc2021EXCR.tides <- left_join(arc2021EXCR, springneapcycles, by ="dates")

arc2021EXCR.tides <- rename(arc2021EXCR.tides, "deep.rates" = "excr deep rates")
arc2021EXCR.tides <- rename(arc2021EXCR.tides, "surface.rates" = "EXCR SURFACE RATES")
...
arc2021EXCR.tides %>% 
  ggplot()+
  geom_point(aes(x=dates,y=surface.rates))+
  geom_line(aes(x=dates,y=time.st))
...
arc2021EXCR.tides %>% 
  ggplot()+
  geom_point(aes(x=dates,y=deep.rates*40))+
  geom_line(aes(x=dates,y=winddir))

arc2021EXCR.tides %>% 
  ggplot()+
  geom_col(aes(x=dates,y=Windspd*20))+
  geom_point(aes(x=dates,y=surface.rates*60),colour="lightblue")+
  geom_point(aes(x=dates,y=winddir),colour="blue")+
  geom_line(aes(x=dates,y=winddir))
